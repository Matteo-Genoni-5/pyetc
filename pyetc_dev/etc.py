...
class ETC:
    ...
    def build_obs_full(self, fo):
        ...
        if fo['COADDXY'] < 1:
            raise ValueError('COADDXY must be >= 1')

        if fo["OBJ"] == 'sb':
            inter_dict["IMA_AREA"] = (fo["COADDXY"]*CONF['spaxel_size'])**2
            inter_dict["IMA_HSIZE"] = None
        elif fo["OBJ"] == 'ps' or fo["OBJ"] == 'resolved':
            if fo["COADDXY"] % 2 == 0:
                raise ValueError('Cannot coadd number of even pixels per side for resolved or point source')
            else:
                inter_dict["IMA_HSIZE"] = int((fo["COADDXY"] - 1) / 2)    

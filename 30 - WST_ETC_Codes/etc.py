...
class ETC:
    ...
    def fiber_aperture_convolution(self, ins, ima, displacement=0.0, print_info=False):
        """
        ...
        # Check if aperture is defined in instrument
        #if 'aperture' not in ins:
        #    raise ValueError("Instrument configuration must contain 'aperture' parameter")
    ...
    def time_from_source_MOS(self, ins, ima, spec, debug=True, sat=True, dit=False):
            ...
            if (obs['ima_type'] == 'sb'):
                ...
                frac_flux = np.pi*(ins['aperture']/2)**2 * tflux 
                # This is the area for the total flux, not the fraction, 
                # this will multiply the spectrum and since that is in 
                # erg/s/cm2/AA/arcsec**2 and this is arcsec**2
            ...
            if dit:
                print('Computing DIT for target SNR:', target_snr)
                nph_source = obs['ndit'] * frac_flux * spec_value * Kt

                source_noise_sq = nph_source
            
                # # # sky
                nph_sky = obs['ndit'] * ins_sky * ins_ins * tel_eff_area * np.pi*(ins['aperture']/2)**2 * (dl/1e4)
                sky_noise_sq = nph_sky

                # # # # ron and dark
                ron_noise_sq = ins['ron']**2*n_vertical*obs['ndit']
                dark_noise_sq = ins['dcurrent']*n_vertical*obs['ndit']/3600
    ...
    def snr_from_source_MOS(self, ins, ima, spec, debug=True):
        ...
        if (obs['ima_type'] == 'sb'):
            # similar to what we do for image case in fiber_aperture_convolution
            frac_flux = np.pi*(ins['aperture']/2)**2 #This is the area of the fiber in arcsec**2, we multiply this for the spec which is in .../arcsec**2
            ...
            if debug:
                ...
                self.logger.debug('frac_flux & tspec shape: %s & %s', np.shape(frac_flux)[0], np.shape(tspec.data)[0])

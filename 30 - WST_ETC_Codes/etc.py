...
# Factor of N x actual spaxels in the aperture, for the MOS
 # # #
FF = 2
# # # 
...
class ETC:
    ...
    def get_spec(self, ins, dspec, oversamp=10, lsfconv=True):
        ...
        if dspec['type'] == "flatcont":
            ...
            l0,dl = dspec['wave_line_center'],dspec['wave_width']
    ...
    def get_ima(self, ins, dima, oversamp=10, uneven=1):
        ...
        if dima['type'] == 'moffat':
            ...
            ima = sersic(ins['spaxel_size'], dima['reff'], dima['n'], dima.get('ell',0),
                         kreff=dima.get('kreff',5), oversamp=oversamp, uneven=uneven)            
        else:
    ...
    def time_from_source(self, ins, ima, spec, debug=True, sat=True, dit=False):
            """ main routine to perform the NDIT/DIT computation for a given source
            ...
            if obs['spec_type'] == 'line':
                target_wave = obs['wave_line_center'] 
            else:
            ...
            if dit:
                ...
                res = {}
                def equation(x):  # x is the DIT we are searching for
                    # Recalculate ALL terms that depend on DIT (x)
                    if obs['ima_type'] == 'sb':
                        nph_source_x = x * obs['ndit'] * tflux * nspaxels * spec_value * Kt
                    else:
                        nph_source_x = x * obs['ndit'] * tflux * frac_flux * spec_value * Kt
                    
                    nph_sky_x = x * obs['ndit'] * ins_sky * ins_ins * tel_eff_area * spaxel_area * nspaxels * (dl/1e4)
                    dark_noise_sq_x = ins['dcurrent'] * nspaxels * obs['ndit'] * x / 3600
                    source_noise_sq_x = nph_source_x
                    sky_noise_sq_x = nph_sky_x
                    ron_noise_sq = ins['ron']**2 * nspaxels * obs['ndit']  # Only this doesn't depend on DIT
                    
                    # Total noise squared
                    tot_noise_sq_x = ron_noise_sq + dark_noise_sq_x + sky_noise_sq_x + source_noise_sq_x
                    
                    # SNR equation: target_snr - actual_snr = 0
                    # # # s p e c  b i n n i n g  a p p r o x
                    if obs['spbin'] > 1:
                        snr_x = nph_source_x / np.sqrt(tot_noise_sq_x) * np.sqrt(obs['spbin'])
                    else:
                        snr_x = nph_source_x / np.sqrt(tot_noise_sq_x)
                    return target_snr - snr_x

                result = root_scalar(equation, bracket=[0.1, 1e7], method='brentq')
            ...
            # if dit == False means we want to compute the NDIT, default
                ...
                print('Computing NDIT for target SNR:', target_snr)
                if  obs['ima_type'] == 'sb':
                ...
                # # # s p e c  b i n n i n g  a p p r o x
                if obs['spbin'] > 1:
                    res['ndit'] = target_snr**2 * tot_noise_sq / ( nph_source**2 * obs['spbin'] )
                else:
    ...
    # # # MOS handy function for the fraction of flux # # #
    def fiber_aperture_convolution(self, ins, ima, displacement=0.0, print_info=False, plot_result=False):
        """
        ...
        return ima_out, flux_fraction, FF * n_spaxels_vertical

    # # # Time computation for MOS instrument # # # 
    def time_from_source_MOS(self, ins, ima, spec, debug=True, sat=True, dit=False):
            """ main routine to perform the NDIT/DIT computation for a given source, in the MOS case
            ...
            if dit:
                _checkobs(obs, usedobs, ['moon','ndit','snr','airmass','spec_type','ima_type','disp'])
                if obs['ndit'] is None:
                    raise ValueError('obs ndit cannot be None for dit computation')
            else:
                _checkobs(obs, usedobs, ['moon','dit','snr','airmass','spec_type','ima_type','disp'])
                if obs['dit'] is None:
                    raise ValueError('obs dit cannot be None for ndit computation')
            ...
            n_vertical = FF * int(ins['aperture'] / ins['spaxel_size'] + 0.5)

            if (ima is None):
                if (obs['ima_type'] != 'ps') and (obs['ima_type'] != 'sb'):
                    raise ValueError('ima can be None only for ps or sb obs ima_type')

            if (obs['ima_type'] == 'ps') or (obs['ima_type'] == 'resolved'):
                fwhm = get_seeing_fwhm(obs['seeing'], obs['airmass'], wave_value, self.tel['diameter'], ins['iq_fwhm_tel'], ins['iq_fwhm_ins'])[0]
                psf = moffat(ins['spaxel_size'], fwhm, ins['iq_beta'], oversamp=10)
                psf.oversamp = 10

                if obs['ima_type'] == 'resolved':
                    dummy = moffat_image(fwhm=(1,1), n=10, shape=ima.shape, flux=1.0, unit_fwhm=None)
                    # we convolve the PSF with the original image
                    conv_ima = ima.copy()
                    conv_ima.data = fftconvolve(ima.data, psf.data, mode='same')
                    conv_ima.data /= conv_ima.data.sum()
                    conv_ima.wcs = dummy.wcs
                    conv_ima.oversamp = psf.oversamp
                    _, frac_flux, _ = self.fiber_aperture_convolution(ins, conv_ima, displacement=obs['disp'], print_info=False)
                
                elif obs['ima_type'] == 'ps':
                    _, frac_flux, _ = self.fiber_aperture_convolution(ins, psf, displacement=obs['disp'], print_info=False)

            if (obs['ima_type'] == 'sb'):
                # similar to what we do for image case in fiber_aperture_convolution
                # For surface brightness, fraction is always 1
                frac_flux = 1  # fraction of flux captured (always 1 for sb)
                tflux *= np.pi*(ins['aperture']/2)**2  # convert from per arcsec² to per fiber
            ...
            if dit:
                ...
                # Define equation to solve: target_snr = nph_source / sqrt(tot_noise²)
                res = {}
                def equation(x):  # x is the DIT we are searching for
                    # All terms that depend on DIT (x)
                    nph_source_x = x * obs['ndit'] * tflux * frac_flux * spec_value * Kt
                    nph_sky_x = x * obs['ndit'] * ins_sky * ins_ins * tel_eff_area * np.pi*(ins['aperture']/2)**2 * (dl/1e4)
                    dark_noise_sq_x = ins['dcurrent'] * n_vertical * obs['ndit'] * x / 3600
                    
                    # CORREZIONE: RON non dipende dal DIT, solo dall'NDIT
                    ron_noise_sq = ins['ron']**2 * n_vertical * obs['ndit']
                    
                    # Total noise squared
                    tot_noise_sq_x = ron_noise_sq + dark_noise_sq_x + nph_sky_x + nph_source_x
                ...
                print(f"Computed DIT: {res['dit']:.2f} > {np.ceil(res['dit']):.2f} for target SNR: {target_snr:.2f} at wavelength {target_wave:.2f} A, in {n_vertical} vertical pixels")
            ...
            # if dit == False means we want to compute the NDIT, default
                ...
                # # # sky
                nph_sky = obs['dit'] * ins_sky * ins_ins * tel_eff_area * np.pi*(ins['aperture']/2)**2 * (dl/1e4)
                sky_noise_sq = nph_sky

                # # # # ron and dark
                ron_noise_sq = ins['ron']**2*n_vertical 
                dark_noise_sq = ins['dcurrent']*n_vertical*obs['dit']/3600
                ...
                print(f"Computed NDIT: {res['ndit']:.2f} > {np.ceil(res['ndit']):.2f} for target SNR: {target_snr:.2f} at wavelength {target_wave:.2f} A, in {n_vertical} vertical pixels")
            ...
            if sat:
                ...
                a_tot = (w*1.e-8/(H_cgs*C_cgs)) * (tel_eff_area*1.e4) * (ins_atm_tot.data)
                Kt_tot =  ins_ins_tot.data * a_tot
                nph_sky_spaxel = (obs['dit'] * ins_sky_tot.data * ins_ins_tot.data * np.pi*(ins['aperture']/2)**2 * (dl/1e4)) / n_vertical

                #equal in every spaxel, we just compute it once
                nph_source_spaxel = obs['dit'] * frac_flux * spec.data * Kt_tot / n_vertical
                tot_counts = nph_source_spaxel + nph_sky_spaxel + ins['ron'] + ins['dcurrent'] * obs['dit']/3600
                frac_sat = np.sum(tot_counts > sat_level) / len_waves
    ...
    # # # SNR computation for MOS instrument # # # 
    def snr_from_source_MOS(self, ins, ima, spec, debug=True):
        """ main routine to perform the S/N computation for a given source > > MOS case only, easier than modifying the IFS one < <
        ...
        if ins['type'] != 'MOS':
            raise ValueError('snr_from_source_MOS only works for MOS instrument')
        ...
        obs = self.obs
        usedobs = {}
        _checkobs(obs, usedobs, ['moon','dit','ndit','airmass','spec_type','ima_type','disp'])
        flux = 1
        tflux = flux
        is_cont = False
        is_line = False
        if obs['spec_type'] == 'cont': # flux is in erg/s/cm2/A
            ...
            if obs['spec_range_type'] == 'fixed':
                ...
                raise NotImplementedError('Adaptative truncation not implemented for MOS')
            else:
        ...
        n_vertical = FF * (ins['aperture'] / ins['spaxel_size']) #int(ins['aperture'] / ins['spaxel_size'] + 0.5)

        if (obs['ima_type'] == 'sb'):
            # similar to what we do for image case in fiber_aperture_convolution
            frac_flux = 1
            tflux *= np.pi*(ins['aperture']/2)**2 # multiply by fiber area in arcsec²
            # This converts from erg/s/cm2/AA/arcsec² to erg/s/cm2/AA per fiber
            
        elif (obs['ima_type'] == 'ps') or (obs['ima_type'] == 'resolved'):
            # # # # Get wavelength array of the cutted (or not cutted) spectrum
            waves = tspec.wave.coord()
            n_waves = len(waves)

            # small spectrum as for the line case
            if len(waves) > 100:
                ...
                for wave in interp_waves:
                    ...
                    if obs['ima_type'] == 'resolved':
                        ...
                        conv_ima = psf.copy()
                        conv_ima.wcs = psf.wcs
                        conv_ima.oversamp = psf.oversamp
                    
                    # the number of spaxels is not needed, just computed once and is constant, as n_vertical, in the future could be variable if needed
                    _, frac_flux_raw, _ = self.fiber_aperture_convolution(ins, conv_ima, displacement=obs['disp'], print_info=False)
                
                    frac_flux_raw_list.append(frac_flux_raw)

                frac_flux = np.interp(waves, interp_waves, frac_flux_raw_list)

            # For line spectra, use the wavelength closest to wave_center
            else:
                
                wave_center = obs['wave_line_center']
                central_idx = np.argmin(np.abs(waves - wave_center))
                wave = waves[central_idx]
                
                # Get PSF at the central wavelength
                psf = self.get_image_psf(ins, wave)
                ...
                # Compute flux fraction once for the central wavelength
                _, frac_flux, _ = self.fiber_aperture_convolution(ins, conv_ima, displacement=obs['disp'], print_info=False)

        if debug:
            self.logger.debug('ima_type & spec are: %s & %s', obs['ima_type' ], obs['spec_type'])
            self.logger.debug('frac_flux & tspec shape: %s & %s', np.shape(frac_flux), np.shape(tspec.data))
            self.logger.debug('number of coadded vertical pixels: %s', n_vertical)

        # # # S N R # # # 
        # # # # # # # # # 

        # # # similar to the ps spectrum case in the IFS
        moon = obs['moon']
        ...
        # truncate instrans and sky to the wavelength limit of tspec
        ins_sky = sky_emi.subspec(lmin=tspec.wave.get_start(), lmax=tspec.wave.get_end())
        ins_ins = ins['instrans'].subspec(lmin=tspec.wave.get_start(), lmax=tspec.wave.get_end())
        ins_atm = sky_abs.subspec(lmin=tspec.wave.get_start(), lmax=tspec.wave.get_end())

        dl = tspec.wave.get_step(unit='Angstrom')
        w = tspec.wave.coord() # wavelength in A

        tel_eff_area = self.tel['area'] * (1 - ins.get('obscuration', 0)) # telescope effective area
        a = (w*1.e-8/(H_cgs*C_cgs)) * (tel_eff_area*1.e4) * (ins_atm.data)
        Kt =  ins_ins * a

        # total number photons from the source, in the vertical pixels
        nph_source = tspec.copy()
        # # # frac_flux should be a 1D array long as tspec.data or a scalar
        nph_source.data = obs['dit'] * obs['ndit'] * Kt.data * tspec.data * frac_flux * tflux
        # # #

        # number of photons received from the sky, collected in the fiber aperture area
        nph_sky = ins_sky * ins_ins * obs['dit'] * obs['ndit'] * tel_eff_area * (dl/1e4) * np.pi*(ins['aperture']/2)**2

        # RON
        ron_noise = tspec.copy()
        ron_noise.data = np.sqrt(ins['ron']**2*np.full_like(ron_noise.data, n_vertical)*obs['ndit'])

        # Dark current
        dark_noise = tspec.copy()
        dark_noise.data = np.sqrt(ins['dcurrent']*np.full_like(dark_noise.data, n_vertical)*obs['ndit']*obs['dit']/3600)
        ...
        # Source noise
        source_noise = tspec.copy()
        source_noise.data = np.sqrt(nph_source.data)

        # Sky noise
        tot_noise = tspec.copy()
        ...
        res = {}
        res['input'] = dict(atm_abs=ins_atm, ins_trans=ins_ins, atm_emi=ins_sky, flux_source=tspec)
        ...
        if debug:
            self.logger.debug('Computation for the MOS done. Source type %s & %s Flux %.2e S/N %.1f FracFlux %.3f Npixels vertical %d',
                            obs['ima_type'], 
                            obs['spec_type'], 
                            flux,
    ...
    def build_obs_full(self, fo):
        ...
        if fo['COADD_XY'] < 1:
            raise ValueError('COADD_XY must be >= 1')

        if fo['Obj_Spat_Dis'] == 'sb':
            inter_dict["IMA_AREA"] = (fo["COADD_XY"]*CONF['spaxel_size'])**2
            inter_dict["IMA_HSIZE"] = 0
        elif fo['Obj_Spat_Dis'] == 'ps' or fo['Obj_Spat_Dis'] == 'resolved':
            if fo["COADD_XY"] % 2 == 0:
                raise ValueError('Cannot coadd number of even pixels per side for resolved or point source')
            else:
                inter_dict["IMA_HSIZE"] = int((fo["COADD_XY"] - 1) / 2)    
                inter_dict["IMA_AREA"] = 0
        ...
        obs = dict(
            ...
            ima_type = fo['Obj_Spat_Dis'],
            ima_area = inter_dict["IMA_AREA"],
            ima_aperture_type = fo["IMA_RANGE"],
            ima_kfwhm = fo["IMA_KFWHM"],
            ima_aperture_hsize_spaxels = inter_dict["IMA_HSIZE"],
            skycalc = fo["SKYCALC"],
            wave_line_center = fo['SEL_CWAV'],

            snr = fo["SNR"],
            snr_wave = fo["Lam_Ref"],

            disp = fo['OBJ_FIB_DISP']
        )
        ...
        # # # s p e c  b i n n i n g  a p p r o x
        #if ('COADD_WL' in fo) and isinstance(fo['COADD_WL'], int) and fo['COADD_WL'] > 1:
        obs["spbin"] = fo["COADD_WL"]
        ...
        if (fo["OPT_IMA"]) & (fo["IMA_RANGE"] == "circular_adaptative") & (fo['Obj_Spat_Dis'] in ('ps', 'resolved')):
            A = CONF['lbda1']
            B = CONF['lbda2']
    ...
    def get_spec2(self, fo):
        ...
        if fo['Obj_SED'] == 'template':
            name, DEFAULT_WAVE, flux = sed_models.template(f"{fo['SED_Name']}.dat")
            redshift = fo['Z']
            band = fo['MAG_FIL']
            mag = fo['OBJ_MAG']
            syst = fo['MAG_SYS']
        ...
        elif fo['Obj_SED'] == 'line':
            DEFAULT_WAVE = np.linspace(100, 30000, 10000)
            center = fo['SEL_CWAV']
            fwhm = fo['SEL_FWHM']
            
            check_line(center, fwhm, l1, l2)
            tot_flux = fo['SEL_FLUX']
...
def moffat(samp, fwhm, beta, ell=0, kfwhm=5, oversamp=1, uneven=1):
...
def sersic(samp, reff, n, ell=0, kreff=5, oversamp=1, uneven=1):
